...
// stop cancels the context of any run in progress and returns once every job
// has returned.
func Start(ctx context.Context, logger *slog.Logger, jobs ...Job) (stop func()) {
	ctx, cancel := context.WithCancel(ctx)
	var wg sync.WaitGroup
	for _, job := range jobs {
		wg.Go(func() { loop(ctx, logger, job) })
	}
	return func() {
		cancel()
		wg.Wait()
...
func loop(ctx context.Context, logger *slog.Logger, job Job) {
	ticker := time.NewTicker(job.Every)
	defer ticker.Stop()
	for {
		runOnce(ctx, logger, job)
		select {
		case <-ctx.Done():
			return
		case <-ticker.C:
...
func runOnce(ctx context.Context, logger *slog.Logger, job Job) {
	start := time.Now()
	defer func() {

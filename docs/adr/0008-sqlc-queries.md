...
## Context
Most database access has a fixed shape: tenants, users, API keys, alert rules and alerts, retention and batch inserts. Writing `rows.Scan` by hand for each is slow, and mistakes only show up at runtime. Search is the exception: any combination of optional filters, sort order and pagination. The dashboard's Top-N and timeline counts take the same optional filters, and evaluating an alert rule depends on which filters the rule sets.
...
- The dashboard counts in `internal/store/counts.go` reuse search's filter conditions rather than being generated, so a filter means the same thing in a search and in the chart beside it. Alert rule evaluation in `internal/store/alerts.go` is built the same way. Columns come from allow-lists here too.
...
- Data access has two styles. The dynamic one is confined to search, the dashboard counts and alert evaluation, each with its own tests.

...
- Ingest sets the tenant for each group of events before inserting, so `WITH CHECK` rejects rows for any other tenant. The tenant comes from the event body, trusted only from the ingest key or an admin (ADR 0009).
